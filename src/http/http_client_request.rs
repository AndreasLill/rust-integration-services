...
impl HttpClientRequest {
    pub fn new(config: Arc<HttpClientConfig>, tls_config: Arc<ClientConfig>, request: HttpRequest) -> Self {
        Self {
            config,
            tls_config,
            request,
    ...
    async fn send_tcp(self, url: Uri) -> anyhow::Result<HttpResponse> {
        ...
        let req = Self::build_http_request(url, self.request, Version::HTTP_11).await?;
    ...
    async fn send_tls(self, url: Uri) -> anyhow::Result<HttpResponse> {
        ...
        match version {
            Version::HTTP_2 => {
                ...
                let req = Self::build_http_request(url, self.request, Version::HTTP_2).await?;
                let res = sender.send_request(req).await?;
                let response = Self::build_http_response(res).await?;
